---
layout: default
title: Home
---

<link rel="stylesheet" href="{{ '/assets/css/style.css' | relative_url }}">

<div class="nav-inline">
  <a href="{{ '/' | relative_url }}">Home</a>
  <a href="{{ '/projects' | relative_url }}">Projects</a>
  <a href="{{ '/articles' | relative_url }}">Articles</a>
</div>

# Venkat Potamsetti
*Leveraging technology to transform healthcare*

<p align="center">
  <img src="https://github.com/potamv01.png" width="120" style="border-radius:50%" alt="Venkat">
</p>

<p>
  <a href="https://github.com/potamv01/VenkatPotamsetti/raw/main/Venkat%20updated%20CV%20Document.pdf" class="btn">Download CV (PDF)</a>
  <a href="https://github.com/potamv01?tab=repositories" class="btn" style="background:#0e75b6">GitHub Projects</a>
  <a href="https://www.linkedin.com/in/venkat-potamsetti-dataanalyst" class="btn" style="background:#0A66C2">LinkedIn</a>
</p>

<p class="callout">
  Clinical strategist & analytics leader turning complex operational data into decisions. I blend SQL/Python, BI, and SOPs to deliver reliable reporting, automation, and engagement programmes for health-tech teams.
</p>

## Impact snapshots
<div class="metrics">
  <div class="metric"><strong>+20% budget</strong><br>through transparent KPIs and delivery governance.</div>
  <div class="metric"><strong>-30% admin time</strong><br>via referral pathway automation and SLA tracking.</div>
  <div class="metric"><strong>+25% survey capture</strong><br>with engagement ops and uplift measurement.</div>
</div>

## Skills & focus areas
<span class="skill">Advanced Excel</span>
<span class="skill">Power BI</span>
<span class="skill">SQL / SQL Server</span>
<span class="skill">Python</span>
<span class="skill">Tableau</span>
<span class="skill">ETL & Data Modelling</span>
<span class="skill">Governance & SOPs</span>
<span class="skill">PRINCE2</span>

## Healthcare lens
- Referral pathways, wait-time and utilisation analytics
- KPI dashboards, benchmarking, and provider performance reviews
- Privacy-safe demos and stakeholder-friendly comms decks

## How I deliver
- Rapid discovery → “to-be” workflow design → incremental releases
- Governance first: reproducible notebooks, SOPs, and data quality checks
- Collaboration with clinical, ops, and product stakeholders to embed adoption

## Availability
<p>
  <a class="btn" style="background:#198754" href="https://calendar.app.google/i3Mj3G1QhCxMrE4g8" target="_blank">View my Calendar</a>
  <a class="btn" href="mailto:venkatpotamsetti@gmail.com?subject=Hello%20Venkat">Email me</a>
</p>

**Email:** venkatpotamsetti@gmail.com • **Location:** Ashford, Kent, UK.
